    ...
    return (
        <div className="w-64 bg-slate-50 border-r border-slate-200 flex flex-col h-full shadow-inner">
            <div className="p-4 border-b border-slate-200 bg-white">
                <button
                    onClick={onNewChat}
                    className="w-full py-3 bg-jobird-red text-white font-black uppercase text-[11px] tracking-widest shadow-md hover:bg-red-700 transition-all flex items-center justify-center gap-2"
                >
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-2">History</div>
                {sessions.map((session) => (
                    ...
                    >
                        <div className={`text-[12px] font-bold truncate pr-6 ${activeSessionId === session.id ? 'text-jobird-red' : 'text-slate-600'
                            }`}>
                            {session.title}
                        </div>
                ...
                {sessions.length === 0 && (
                    <div className="text-[11px] text-slate-400 italic px-2 py-4">No recent chats</div>
                )}
            </div>
